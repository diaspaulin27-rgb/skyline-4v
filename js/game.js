...
const glassDropsSystem = new GlassDrops(glassCanvas);
    ...
    function renderPhases() {
      ...
      phases.slice(0,ACTIVE_PHASE_COUNT).forEach((p, i) => {
        ...
        div.innerHTML = `<div class="phase-number">${t('phase_hud')} ${i + 1}</div><div class="phase-name">${unlocked ? localize(p.name) + perfectMark : t('phase_locked')}</div><div class="phase-mini-feeling">${unlocked ? localize(p.feeling) : t('phase_req')}</div>`;
